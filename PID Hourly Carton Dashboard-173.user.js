...
(function () {
  ...
  async function fetchPreSortCases(hourStr, dateISO) {
    try {
      ...
      console.log('Fetching PreSort Cases from PPR:', url);

      // Fetch the PPR page
      const html = await gmFetch(url);

      // Parse HTML to extract PreSort cases
      const parser = new DOMParser();
      const doc = parser.parseFromString(html, 'text/html');

      // Extract PreSort Cases volume using CSS selector from IXD_SUMMARY script
      // Selector: tr[id="ppr.detail.da.rcSort.rcSortCases"] td.actualVolume.numeric div.original
      const preSortCasesElement = doc.querySelector('tr[id="ppr.detail.da.rcSort.rcSortCases"] td.actualVolume.numeric div.original');
      const preSortCases = preSortCasesElement ? Math.round(extractNumericValue(preSortCasesElement.textContent)) : 0;

      console.log(`PreSort Cases for hour ${hourStr}: ${preSortCases}`);
      return preSortCases;
    } catch (e) {
      console.error('PreSort Cases fetch failed for hour', hourStr, ':', e);
      return 0;
    }
  }
  ...
  function restoreHourlyData() {
    Object.keys(state.hourlyData).forEach(hs => {
      ...
      if (row && state.hourlyData[hs]) {
        const { total, prPallets, prCases, lostTI, dpmo, totalDefects, sortVolume, preSortCases, nvfCartons, transInCartons, tiToteTotal, throughputHours, daCartons, tpCplh } = state.hourlyData[hs];
        ...
        if (typeof preSortCases === 'number') {
          ...
          row.querySelector('.preSortCell').textContent = fmt(preSortCases);
          row.querySelector('.preSortCell').dataset.v = preSortCases;
...
These hours are cached but incomplete. Would you like to clear their cache and re-run them now?`;
  ...
  async function runHour(hs, dateISO) {
    ...
    try {
      ...
      const [cartonData, lostTI, defectData, sortVolume, preSortCases, throughputHours, daCartons] = await Promise.race([fetchAllForHour, hourWatchdog]);
      ...
      console.log(`Sort/PreSort for hour ${hs}: Sort=${sortVolume}, PreSort=${preSortCases}`);
  ...
  function updateTotals(){
    ...
    let baseSortGoal=0, basePreSortGoal=0, sortTotal=0, preSortTotal=0;
